...
We test the two channels of §3.4 (Table `rq1_mean_preserving_spread.md`, Figure `welfare_incidence_price.png`). The **price-variation channel** — learned surge versus a flat price at the *same* average multiplier — reproduces Castillo's structure wherever the average price is in the operating range. For a low-average controller (throughput, mean multiplier $1.04$), the variation channel yields, as a percentage of gross revenue: total welfare $+1.37$, rider surplus $+0.73$, driver surplus $+0.39$, platform profit $+0.25$ — *all positive, with riders gaining most and drivers/platform gaining slightly*, exactly the allocative-efficiency structure Castillo attributes to surge ("surge dominates uniform at every average multiplier; driver surplus and profit are also higher, but only slightly"). The welfare-weighted (Castillo-analog) controller's variation channel is [W1]/[R1]/[D1]/[P1] (W/rider/driver/platform, %GR) [INTERP]. This channel is calibration-robust and essentially objective-independent. The one exception is instructive: at a very high average price (profit, mean $3.43$), the variation channel's rider term turns slightly negative ($-0.57$) because, with most riders already priced out, redistributing the remaining price dispersion cannot help them — a boundary effect, not a reversal of the mechanism.
...
The same surge *mechanism*, optimized for different objectives, produces sharply different incidence on the same market. The **welfare** (planner) controller — by construction the efficiency benchmark — prices moderately (mean multiplier $2.07$), attains the highest total welfare (index 100), and splits it relatively evenly (rider 47% / driver 36% / platform 17%). The two *platform-centric* objectives both sacrifice efficiency *and* skew the distribution to opposite extremes: a **profit** controller prices near the willingness-to-pay ceiling ($3.43$), gives up $\sim$7% of total welfare (index 93), and compresses riders to **11%** of welfare while capturing the rest as driver (62%) and platform (26%) surplus — extraction; a **throughput** controller prices near base ($1.04$), gives up a similar $\sim$7%, and flips the split to **77%** for riders. Two facts make the point. First, profit and throughput achieve essentially the *same* total welfare as each other yet distribute it oppositely — the objective is, between them, almost **purely distributional**. Second, the cross-objective swing in the rider's welfare share — **from 11% to 77%** — dwarfs the $\sim$7% efficiency differences, so *the incidence is overwhelmingly a property of the objective, not of surge.* A platform optimizing a throughput/GMV-style metric (as deployed systems do) sits near the rider-favoring pole; one optimizing profit sits at the extractive pole — and neither metric, on its own, reveals the distribution it implies.
