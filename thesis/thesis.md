...
> *[Detailed description of demand, the heterogeneous strategic driver model, matching/wild-goose-chase, calibration to Castillo's elasticity, and the PPO controller. Draft below; finalize after experiments.]*
...
**Augmented Random Search.** We therefore optimize the controller with ARS-v2 (Mania, Guy & Recht 2018), which estimates a gradient of the *episode* objective by finite differences over random parameter perturbations, with state normalization and top-direction selection. The controller is a linear policy over normalized observations whose output is squashed by the environment to bounded per-zone surge multipliers, a dispatch radius, and per-zone rebalancing incentives. ARS sidesteps per-step credit assignment by scoring whole episodes, and is robust in exactly this regime. Each candidate is evaluated on three rollout seeds; we use 16 directions, the best 8 per update, and parallelize evaluations across processes.
