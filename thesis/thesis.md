...
The **price-level channel** behaves differently in our market than in Castillo's, and the difference is itself informative. Castillo's headline driver/platform *losses* require the platform-optimal uniform price to sit *above* the surge average, so that adopting surge lowers the average price. In our market the learned controllers price *at or slightly above* their objective's uniform optimum (e.g., welfare-weighted surge average $0.93$ vs optimal uniform $0.90$), so the level channel does **not** transfer surplus to riders; the full surge-vs-optimal-uniform comparison (third table in `aggregate_results.py` output) accordingly shows riders roughly flat-to-down and drivers up for the rider-favoring objectives — the *opposite* of the headline. The Castillo headline therefore lives in the **variation** channel here, not the level channel; reproducing the level channel's driver loss requires a deliberate price-level *reduction*, which we construct and analyze in the targeted experiment of §5.3.
...
Second, to test Castillo's mechanism directly we impose his scenario — a price-level *reduction* — in a targeted experiment (`rq3_exposure.py`): a high uniform price ($m=2.0$) versus a peak-targeting surge that keeps peak prices but drops the off-peak price to base, lowering the trip-weighted average to $1.52$. Now drivers lose (riders gain $+17.7\%$ of gross revenue from the cheaper off-peak rides), and the loss falls **most heavily on full-time drivers in absolute per-driver terms** — full-time $-\$21.4$/driver, part-time $-\$20.8$, casual $-\$20.3$ — precisely Castillo's "long-hours drivers hurt most." The mechanism is timing/exposure, not elasticity: full-time drivers (low reservation wage) stay online through the off-peak periods whose price is cut, so they absorb the reduction, whereas casual drivers concentrate at the still-high peak and are partly insulated. (In *percentage* terms casual drivers fall most, $-36\%$ vs $-22\%$, because their base earnings are lowest; the per-driver dollar loss, the quantity comparable to Castillo's per-hour figure, is largest for full-timers.)
