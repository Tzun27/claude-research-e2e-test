...
Ride-hailing platforms increasingly set prices, matching, and rebalancing with learned controllers, but these systems are evaluated only on platform-centric metrics — never on *who* gains and loses. Structural economics has answered the latter for a fixed algorithm: Castillo (2025) finds that surge pricing, relative to an optimized uniform price, raises total welfare and rider surplus while reducing driver surplus and platform profit, hurting the most-engaged drivers most. We ask whether a *learned* controller reproduces this welfare-incidence structure, and how it depends on the controller's objective. We build a calibrated spatial ride-hailing simulator with elastic demand, a heterogeneous strategically-responding driver population, and endogenous matching frictions (a wild-goose chase), and an exact four-way welfare decomposition; controllers are optimized by Augmented Random Search, a derivative-free policy-search method we adopt after a per-step deep-RL method (PPO) failed for a diagnosed credit-assignment reason. Decomposing surge's effect, we find that the **price-variation channel** — spatio-temporal price variation at a fixed average — reproduces Castillo's rider-favoring allocative-efficiency structure: for the Castillo-analog (welfare-weighted) objective it matches three of his four signs (total welfare ↑, rider ↑, driver ↓) with a rider-to-driver gain ratio (≈4.5) close to his (≈3.6). But the **full incidence** is governed not by surge but by the controller's *objective*: across profit-, throughput-, welfare-, and welfare-weighted controllers on the *same* market, the rider's share of total welfare swings from **11% (profit) to 80% (welfare-weighted)** — a seven-fold range — while total welfare varies by under 10%. The objective is thus *predominantly distributional*: it reshapes who captures welfare far more than it changes the total. When surge does lower the average price, drivers as a whole lose and riders gain (+18% of gross revenue), and the loss is nominally largest for the most-engaged (long-hours) drivers, in the direction Castillo's timing/exposure mechanism predicts (though within sampling noise at our scale); a driver-fairness penalty traces an efficiency–equity frontier. The central message is that the distributional incidence of learned surge is a property of the objective, not of surge — making the choice of training objective an implicit, and currently unmeasured, distributional policy.
...
We test the two channels of §3.4 (Table `rq1_mean_preserving_spread.md`, Figure `welfare_incidence_price.png`). The **price-variation channel** — learned surge versus a flat price at the *same* average multiplier — reproduces Castillo's structure across the operating range. For the **welfare-weighted (Castillo-analog)** controller, the variation channel yields, as a percentage of gross revenue: total welfare $+0.50$, rider surplus $+0.63$, driver surplus $-0.14$, platform profit $+0.01$. This matches **three of Castillo's four signs** — total ↑, rider ↑, driver ↓ — with platform approximately flat (Castillo's small $-0.50$); and the *proportion* is right: the rider gain is about $4.5\times$ the driver loss, close to Castillo's ratio of $\approx 3.6$ (rider $+3.57$ vs driver $-0.98$). For the **throughput** controller (mean multiplier $1.04$) the variation channel is all-positive — total $+1.37$, rider $+0.73$, driver $+0.39$, platform $+0.25$ — exactly Castillo's "surge dominates uniform at every average multiplier; driver surplus and profit are also higher, but only slightly." The one boundary case is instructive: at a very high average price (profit, mean $3.43$) the rider term turns slightly negative ($-0.57$) because, with most riders already priced out, redistributing the remaining price dispersion cannot help them — a boundary effect, not a reversal of the mechanism. Across objectives, then, the variation channel robustly delivers Castillo's rider-favoring allocative-efficiency structure.

The **price-level channel** behaves differently in our market than in Castillo's, and the difference is itself informative. Castillo's headline driver/platform *losses* require the platform-optimal uniform price to sit *above* the surge average, so that adopting surge lowers the average price. In our market the learned controllers price *at or slightly above* their objective's uniform optimum (e.g., welfare-weighted surge average $0.93$ vs optimal uniform $0.90$), so the level channel does **not** transfer surplus to riders; the full surge-vs-optimal-uniform comparison (third table in `aggregate_results.py` output) accordingly shows riders *down* (welfare-weighted $-0.9\%$ of GR, throughput $-4.7\%$) and drivers *up* ($+2.2\%$, $+11.1\%$) for the rider-favoring objectives — the *opposite* of Castillo's headline, because here the learned surge raises the average above the (near-floor) uniform optimum rather than lowering it. The Castillo headline therefore lives in the **variation** channel here, not the level channel; reproducing the level channel's driver loss requires a deliberate price-level *reduction*, which we construct and analyze in the targeted experiment of §5.3.
...
Two complementary findings. First, in the *same-objective* surge-vs-uniform comparison (Table `driver_incidence.md`), no driver type loses — for every objective all three types see a small per-capita *gain* (e.g., under throughput, full-time $+8.42$, part-time $+8.48$, casual $+7.41$ per driver). This is the expected consequence of §5.1's level-channel finding: when the learned surge's average tracks the objective's uniform optimum, there is no price-level drop and hence no driver loss to allocate. The "who is hurt most" question is therefore vacuous in this comparison — a clean, if negative, result.

Second, to test Castillo's mechanism directly we impose his scenario — a price-level *reduction* — in a targeted experiment (`rq3_exposure.py`): a high uniform price ($m=2.0$) versus a peak-targeting surge that keeps peak prices but drops the off-peak price to base, lowering the trip-weighted average to $1.52$. The **core Castillo redistribution reproduces clearly**: drivers as a whole lose and riders gain ($+17.7\%$ of gross revenue from the cheaper off-peak rides). The *by-type* differentiation is in the predicted direction but **not robustly resolved at our sample size**: the absolute per-driver loss is nominally largest for full-time drivers ($-\$21.4$ vs part-time $-\$20.8$, casual $-\$20.3$), consistent with the timing/exposure mechanism (full-time drivers, with low reservation wages, stay online through the off-peak periods whose price is cut, while casual drivers concentrate at the still-high peak), but the full-time–casual gap of $\$1.1$/driver is only $\approx0.5$ s.e.m. over 16 seeds. We therefore report the *direction* as consistent with Castillo's "long-hours drivers hurt most" rather than a sharply resolved ordering; the differentiation is weak because, as the next paragraph shows, our driver types do not differ enough in *when* they are online for the exposure channel to bite hard. (The ranking is also normalization-dependent: in percentage of base earnings casual drivers fall most, $-36\%$ vs $-22\%$, because their base is lowest. We use the absolute per-driver loss as the analog to Castillo's per-hour measure but do not claim per-hour equivalence, since our types differ in hours worked.)

A policy-inspection diagnostic (Figure `policy_price_welfare_weighted.png`) explains *why* the targeted experiment is needed to surface this channel: the learned welfare-weighted controller varies its surge multiplier **spatially** (across zones, range $0.85$–$1.13$) but is essentially **flat over time** (correlation between mean multiplier and demand $\approx 0.02$; peak-epoch mean $0.92$ versus off-peak $0.92$). The rider-weighted objective has little reason to raise prices at peaks — doing so would cut the rider surplus it is rewarded for — so it forgoes temporal price discrimination, leaving the timing/exposure channel dormant in the *learned* policy. The channel is real (the targeted experiment activates it) but the learned controller does not exercise it; this is a substantive observation about what objective-driven learning does and does not discover, not a limitation of the mechanism.
...
The learned controller reproduces Castillo's **price-variation** structure cleanly: at operating-range average prices, surge versus a same-average flat price lifts total welfare and rider surplus most, with drivers and platform gaining slightly (§5.1) — Castillo's "surge dominates uniform at every average; driver surplus and profit are also higher, but only slightly." The *mechanism* is identical: surge screens scarce capacity toward higher-value riders and curbs the peak crunch, and drivers do not share in the allocative gain because they are paid by the meter.

Where we diverge from Castillo's *headline* is the **price-level** effect, and the divergence is itself the thesis's point. Castillo's net driver and platform *losses* require the platform-optimal uniform price to sit *above* the surge average (his $1.174$); the move to surge then lowers the average price and transfers surplus to riders. In our proxy the welfare-weighted platform's optimal uniform price sits near the base fare ($\approx0.9$, §4.4), and the learned surge's average ($0.93$) sits *just above* it, so the downward level shift is absent for the Castillo-analog objective: the full surge-vs-optimal-uniform comparison shows drivers *gaining* ($+2.2\%$ of GR) and riders roughly flat-to-down ($-0.9\%$), rather than Castillo's driver loss. The driver/platform losses re-appear exactly when we force a price-level *reduction* (the §5.3 targeted experiment) or, equivalently, would appear for an objective whose optimal uniform price is high enough that the learned surge undercuts it — confirming that the headline incidence is a property of the price level the objective induces, not of surge per se. A second, modeling divergence: our imbalance runs largely on the extensive margin (peak abandonment) plus a reduced-form congestion delay, whereas Castillo's stated main channel is intensive-margin pickup time (§6.3); the alignment we claim is at the level of welfare *incidence*, not the micro-mechanics of matching.
...
The answers: **(RQ1)** learned surge reproduces Castillo's rider-favoring structure through the *price-variation* channel — the Castillo-analog controller matches three of his four welfare signs with a comparable rider-to-driver proportion — while the price-*level* channel, and thus the headline driver/platform losses, depends on the objective's price level. **(RQ2, the central result)** the incidence is overwhelmingly a property of the objective, not of surge: the rider's share of total welfare swings from 11% under a profit objective to 80% under a welfare-weighted one on the same market, while total welfare moves under 10% — the objective is almost purely distributional, and the social-planner (welfare) objective is both the most efficient and the most balanced. **(RQ3)** when surge does lower the average price, drivers lose and riders gain, with the loss nominally largest for the most-engaged (long-hours) drivers — the direction of Castillo's timing/exposure mechanism, though the by-type ordering is within sampling noise at our scale. **(RQ4)** a driver-fairness penalty trades efficiency for driver equity along a frontier (§5.4). Together these reframe the deployed-systems practice of optimizing platform-centric metrics (GMV, completion rate) as an implicit, and currently unmeasured, distributional choice.
